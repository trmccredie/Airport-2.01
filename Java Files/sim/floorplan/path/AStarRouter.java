...
public class AStarRouter {
    ...
    /**
     * Find a path between two points (image pixel coords).
     *
     * @param mask walkable map
     * @param startPx start in image pixels
     * @param goalPx goal in image pixels
     * @param stridePx grid stride in pixels (>=1). Larger = faster, less detailed.
     * @param maxExpanded safety cap to avoid runaway on giant masks
     * @param allowDiagonal allow 8-neighbor moves
     */
    public static List<Point> findPath(
        ...
        while (!open.isEmpty() && expanded < maxExpanded) {
            ...
            if (cIdx == goalIdx) {
                return simplifyCollinear(reconstructPath(cameFrom, gw, stride, w, h, goalIdx));
            }
            ...
            for (int k = 0; k < dx.length; k++) {
                ...
                // Pixel coordinate of this coarse node
                int px = Math.min(w - 1, nx * stride);
                int py = Math.min(h - 1, ny * stride);

                if (!mask.isWalkable(px, py)) continue;
    ...
    /** Keep only turning points (fast + good enough for “proof overlay”). */
    private static List<Point> simplifyCollinear(List<Point> path) {
        ...
        for (int i = 1; i < path.size(); i++) {
            ...
            if (dx == lastDx && dy == lastDy) {
                // same direction: replace last point (extend)
