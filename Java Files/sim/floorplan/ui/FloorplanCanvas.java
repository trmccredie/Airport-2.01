...
public class FloorplanCanvas extends JPanel {
    ...
    public enum Tool {
        ...
        // ✅ Milestone 5: Test Route overlay tool
        TEST_ROUTE
    }
    ...
    // editing lock (allows pan/zoom + selection + test route, blocks edits)
    ...
    // callbacks
    private BiConsumer<Tool, Point> onPointAction;        // for PLACE_* and TEST_ROUTE clicks
    private BiConsumer<Tool, Polygon> onPolygonFinished;  // for DRAW_* completion
        ...
        MouseAdapter ma = new MouseAdapter() {
            ...
            @Override public void mousePressed(MouseEvent e) {
                ...
                if (SwingUtilities.isLeftMouseButton(e)) {
                    ...
                    // ✅ TEST ROUTE (allowed even when locked)
                    if (tool == Tool.TEST_ROUTE) {
                        Point imgPt = screenToImage(e.getPoint());
                    ...
                    // POLY tools
                    if (isAnyPolygonTool(tool)) {
                        if (locked && tool != Tool.SELECT) {
                            // locked blocks poly-fill/draw
                            return;
                        }
                        Point imgPt = screenToImage(e.getPoint());
                        polyPts.add(imgPt);
    ...
    public void setImage(BufferedImage img) {
        ...
        // route overlay remains unless explicitly cleared by editor,
        // but if you prefer auto-clear on new image:
        clearTestRoute();
    ...
    public void setTool(Tool t) {
        ...
        // leaving any polygon tool clears partial poly
    ...
    private Zone hitTest(Point imgPt, double anchorRadiusPx) {
        ...
        // 1) nearest anchor within radius
        ...
        // 2) polygon contains point (prefer smallest area)
        Zone bestPoly = null;
    ...
    // ==========================================================
    // Polygon actions (poly-fill OR draw-polygons)
    // ==========================================================
    ...
    private void closePolygonAction() {
        ...
        if (tool == Tool.POLY_FILL_WALKABLE) {
            if (!locked) fillPolygon(polyPts, true);
    ...
    private void fillPolygon(List<Point> ptsImg, boolean makeWalkable) {
        ...
        // rasterize polygon into a small bitmap
        ...
        // apply to mask + overlay
        int changed = 0;
        for (int y = 0; y < bh; y++) {
            for (int x = 0; x < bw; x++) {
                int rgb = polyImg.getRGB(x, y) & 0xFFFFFF;
                if (rgb == 0) continue; // outside polygon
    ...
    protected void paintComponent(Graphics g) {
        ...
        try {
            ...
            // ✅ Draw Test Route overlay (image coords)
            drawTestRoute(g2);

            // Draw polygon-in-progress (image coords)
            if (isAnyPolygonTool(tool) && !polyPts.isEmpty()) {
